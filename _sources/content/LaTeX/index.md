...
When making figures, I usually use Python to save pdf files from `matplotlib`. While an installed version of LaTeX is required to compile mathtext, it ensures: any text has consistent formatting with the final document; is searchable; and if the figure size is set correctly then the font size is also consistent. The width of the page in which text can be written in a tex file is obtained using `\linewidth`, however when this is included in a document it returns a value in pts. I use the following command which I found in [this thread](https://tex.stackexchange.com/questions/8260/what-are-the-various-units-ex-em-in-pt-bp-dd-pc-expressed-in-mm) to convert.
